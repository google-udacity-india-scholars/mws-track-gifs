...
/**
 * @description Get all GIFS
 * @author Istiaque Siddiqi
 */
const fetchGifsByQuery = async () => {
    try {
        let query = getParameterByName('q');

        if (!query) { // no q name found in URL
            throw (Error('No q in URL'));
        } else {
            let gifs = await DBHelper.getGifsByCategoryName(query);
            setSearchResults(gifs.data, query);
            loadLazyImage();
        }
    } catch (error) {
        logErrorMsg(error, `fetchGifsByQuery`);
    }
}
...
/**
 * 
 * @description Set all searched gifs
...
/**
 * @description Create favorite gifs cards
